/**
 * Professional SVG chess piece definitions.
 * Based on the classic Cburnett / Wikimedia Commons chess piece set (public domain).
 * Each piece is a React component rendering an inline SVG at 100% of its container.
 */
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 45 45" className={className}>
      <g fill="#fff" stroke="#000" strokeWidth="1.5" strokeLinejoin="round">
        <path d="M8 12a2 2 0 1 1-4 0 2 2 0 1 1 4 0zm16.5-4.5a2 2 0 1 1-4 0 2 2 0 1 1 4 0zm-16 0a2 2 0 1 1-4 0 2 2 0 1 1 4 0zm32 0a2 2 0 1 1-4 0 2 2 0 1 1 4 0zm-16 0a2 2 0 1 1-4 0 2 2 0 1 1 4 0z" />
        <path d="M9 26c8.5-1.5 21-1.5 27 0l2-12-7 11V11l-5.5 13.5-3-15-3 15L14 11v14L7 14l2 12z" strokeLinecap="butt" />
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 45 45" className={className}>
      <g fill="#fff" fillRule="evenodd" stroke="#000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M12 35.5h21m-20-4h19m-18-2h17" fill="none" stroke="#000" strokeLinejoin="miter" />
      </g>
    </svg>
  );
}
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 45 45" className={className}>
      <g fill="#000" stroke="#000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M11 29a35 35 1 0 1 23 0m-21.5 2.5h20m-21 3h21" fill="none" stroke="#fff" />
      </g>
    </svg>
  );
}
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 45 45" className={className}>
      <g fillRule="evenodd" stroke="#000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        <path d="M9 39h27v-3H9v3zm3.5-7l1.5-2.5h17l1.5 2.5h-20zm-.5 4v-4h21v4H12z" strokeLinecap="butt" fill="#000" />
        ...
        <path d="M12 35.5h21m-20-4h19m-18-2h17" fill="none" stroke="#fff" strokeWidth="1" strokeLinejoin="miter" />
      </g>
    </svg>
  );
}
...
import type { PieceStyle } from '../../store/useSettingsStore';

const PIECE_COMPONENTS: Record<string, ComponentType<PieceSvgProps>> = {
...
// Modern style - slightly different fill/stroke
function ModernPieceWrapper({ Component, className }: { Component: ComponentType<PieceSvgProps>; className?: string }) {
  return (
    <div className={`${className} opacity-90`} style={{ filter: 'contrast(1.1)' }}>
      <Component className="h-full w-full" />
    </div>
  );
}

// Minimal style - simpler geometric shapes
function MinimalPieceWrapper({ Component, className }: { Component: ComponentType<PieceSvgProps>; className?: string }) {
  return (
    <div className={`${className} opacity-85`} style={{ filter: 'brightness(1.1) contrast(0.9)' }}>
      <Component className="h-full w-full" />
    </div>
  );
}

// Pixel style - pixelated effect
function PixelPieceWrapper({ Component, className }: { Component: ComponentType<PieceSvgProps>; className?: string }) {
  return (
    <div className={className} style={{ imageRendering: 'pixelated', filter: 'contrast(1.2)' }}>
      <Component className="h-full w-full" />
    </div>
  );
}

// Alpha style - more refined, cleaner
function AlphaPieceWrapper({ Component, className }: { Component: ComponentType<PieceSvgProps>; className?: string }) {
  return (
    <div className={className} style={{ filter: 'brightness(1.05) saturate(0.95)' }}>
      <Component className="h-full w-full" />
    </div>
  );
}

export function getPieceComponent(color: string, type: string, style: PieceStyle = 'standard'): ComponentType<PieceSvgProps> | undefined {
  const baseComponent = PIECE_COMPONENTS[`${color}-${type}`];
  
  if (!baseComponent) return undefined;
  
  // Return standard pieces as-is
  if (style === 'standard') {
    return baseComponent;
  }
  
  // Return wrapped components for other styles
  switch (style) {
    case 'modern':
      return ({ className }: PieceSvgProps) => <ModernPieceWrapper Component={baseComponent} className={className} />;
    case 'minimal':
      return ({ className }: PieceSvgProps) => <MinimalPieceWrapper Component={baseComponent} className={className} />;
    case 'pixel':
      return ({ className }: PieceSvgProps) => <PixelPieceWrapper Component={baseComponent} className={className} />;
    case 'alpha':
      return ({ className }: PieceSvgProps) => <AlphaPieceWrapper Component={baseComponent} className={className} />;
    default:
      return baseComponent;
  }
}
